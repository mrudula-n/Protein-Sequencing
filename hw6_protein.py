...
def makeAminoAcidLabels(proteinList1, proteinList2):
    genes=[]
    proteinlst1=combineProteins(proteinList1)
    proteinlst2=combineProteins(proteinList2)
    amino_dictionary1=aminoAcidDictionary(proteinlst1)
    amino_dictionary2=aminoAcidDictionary(proteinlst2)
    for i in amino_dictionary1:
        if i not in genes:
            genes.append(i)
    for j in amino_dictionary2:
        if j not in genes:
            genes.append(j)
    genes.sort()
    return genes
...
def setupChartData(labels, proteinList):
    list_of_proteins=combineProteins(proteinList)
    amino_dict=aminoAcidDictionary(list_of_proteins)
    genes=[]
    for i in labels:
        if i in amino_dict:
            genes.append(amino_dict[i]/len(list_of_proteins))
        else:
            genes.append(0)
    return genes
...
def runFullProgram():
    humanProteins=synthesizeProteins("data\human_p53.txt", "data\codon_table.json")
    elephantProteins=synthesizeProteins("data\elephant_p53.txt", "data\codon_table.json")
    proteinsCommon=commonProteins(humanProteins, elephantProteins)
    difference=findAminoAcidDifferences(humanProteins, elephantProteins, 0.005)
    displayTextResults(proteinsCommon, difference)
    aminoLabels=makeAminoAcidLabels(humanProteins, elephantProteins)
    f1=setupChartData(aminoLabels, humanProteins)
    f2=setupChartData(aminoLabels, elephantProteins)
    edges=makeEdgeList(aminoLabels, difference)
    createChart(aminoLabels, f1, "Human", f2, "Elephant", edgeList=edges)
    return
